...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const body = (await req.json()) as { nombre?: string; isActive?: boolean };
    ...
    try {
      const updated = await prisma.cargoCatalog.update({
        where: { id },
        data: data as { nombre?: string; isActive?: boolean },
      });
  ...
  try {
    ...
    await prisma.$transaction([
      prisma.informeDiario.updateMany({
        where: { cargoCatalogoId: id },
        data: { cargoCatalogoId: null },
      }),
      prisma.cargoCatalog.delete({ where: { id } }),
    ]);
