...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const updated = await prisma.contratistaCatalog.update({
      where: { id },
      data: data as any,
    });
  ...
  try {
    ...
    await prisma.$transaction([
      prisma.informeDiario.updateMany({
        where: { contratistaCatalogoId: id },
        data: { contratistaCatalogoId: null },
      }),
      prisma.contratistaCatalog.delete({ where: { id } }),
    ]);
