...
export async function GET(req: NextRequest) {
  try {
    ...
    const items = await prisma.encargadoReporteCatalog.findMany({
    ...
    console.error(error);
    return NextResponse.json({ error: 'Error al listar encargados' }, { status: 500 });
  }
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    try {
      const item = await prisma.encargadoReporteCatalog.create({
      ...
      if (pe.code === 'P2002') {
        return NextResponse.json(
          { error: 'Ya existe un encargado con esa cédula en esta obra' },
          { status: 409 },
        );
      }
      throw e;
    ...
    console.error(error);
    return NextResponse.json({ error: 'Error al crear encargado' }, { status: 500 });
  }
}
