...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const body = (await req.json()) as { codigo?: string; nombre?: string; orden?: number; isActive?: boolean };
    ...
    const chapter = await prisma.budgetChapter.update({ where: { id }, data: data as any });
...
export async function DELETE(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (a) return NextResponse.json({ error: a.error }, { status: a.status });
    const { id } = await params;
    await prisma.budgetChapter.delete({ where: { id } });
